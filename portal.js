(() => {
  ...
  function renderFormsAdmin(root, data) {
    root.innerHTML = adminHeading('FORMS', 'Create form', 'new-form') + `<div class="admin-list">${data.forms.map((form, index) => `<article><div><span class="status-pill">${escapeHtml(isFormOpen(form) ? 'open' : form.status)}</span><h3>${escapeHtml(form.title)}</h3><p>${form.fields.length} fields · ${escapeHtml(form.description)}${form.opensAt || form.closesAt ? ' · Scheduled' : ''}</p></div><div class="admin-row-actions"><button class="icon-button" data-move-form="${form.id}" data-direction="-1" ${index === 0 ? 'disabled' : ''}>↑</button><button class="icon-button" data-move-form="${form.id}" data-direction="1" ${index === data.forms.length - 1 ? 'disabled' : ''}>↓</button><button class="text-link" data-duplicate-form="${form.id}">Duplicate</button><button class="text-link" data-edit-form="${form.id}">Edit</button><button class="icon-button danger" data-delete-form="${form.id}">Delete</button></div></article>`).join('')}</div>`;
    ...
    root.querySelectorAll('[data-duplicate-form]').forEach(button => button.onclick = async () => { const source = data.forms.find(item => item.id === button.dataset.duplicateForm); const copy = structuredClone(source); copy.id = `${source.id}-copy-${Date.now()}`; copy.title = `${source.title} copy`; await saveItem('forms', copy, data, 'Duplicated form'); renderAdminTab('forms'); });
  ...
  function openFormEditor(existing, data) {
    ...
    const form = existing || { id: '', title: '', description: '', status: 'open', fields: [], confirmationMessage: '' };
  ...
  function openSubmissionReview(item, data) {
    const dialog = document.getElementById('admin-dialog'); const body = document.getElementById('admin-dialog-body');
    body.innerHTML = `${adminHeading('REVIEW')}<div class="submission-review-meta"><span><small>Member</small><strong>${escapeHtml(item.user?.global_name || item.user?.username || item.userId || 'Unknown')}</strong></span><span><small>Submitted</small><strong>${new Date(item.createdAt).toLocaleString()}</strong></span></div><div class="answer-list">${Object.entries(item.values).map(([key, value]) => `<div><small>${escapeHtml(key.replaceAll('_', ' '))}</small><p>${escapeHtml(value)}</p></div>`).join('')}</div><div class="field-row"><label class="portal-field"><span>Status</span><select id="submission-status"><option value="received">New</option><option value="in review">Under review</option><option value="approved">Accepted</option><option value="declined">Declined</option><option value="closed">Closed</option></select></label><label class="portal-field"><span>Assigned staff member</span><input id="submission-assignee" value="${escapeHtml(item.assignedTo || '')}" placeholder="Name or callsign" /></label></div><label class="portal-field"><span>Private staff notes</span><textarea id="submission-notes" placeholder="Only staff can see these notes">${escapeHtml(item.staffNotes || '')}</textarea></label>${item.formId === 'suggestion' ? `<label class="portal-field"><span>Public staff response</span><textarea id="submission-response" placeholder="Shown publicly beneath this suggestion">${escapeHtml(item.staffResponse || '')}</textarea></label>` : ''}<button class="button" id="save-submission">Save review</button>`;
    body.querySelector('#submission-status').value = item.status; dialog.showModal(); body.querySelector('#save-submission').onclick = async () => { item.status = body.querySelector('#submission-status').value; item.assignedTo = body.querySelector('#submission-assignee').value.trim(); item.staffNotes = body.querySelector('#submission-notes').value.trim(); if (item.formId === 'suggestion') item.staffResponse = body.querySelector('#submission-response').value.trim(); await saveItem('submissions', item, data, 'Reviewed submission'); dialog.close(); renderAdminTab('submissions'); };
  }

  function renderDepartmentsAdmin(root, data) {
    root.innerHTML = adminHeading('DEPARTMENTS', 'Create page', 'new-department') + `<div class="admin-list">${data.departments.map((item, index) => `<article><div><span class="department-dot" style="background:${escapeHtml(item.accent)}"></span><h3>${escapeHtml(item.name)}</h3><p>/${escapeHtml(item.slug)} · ${escapeHtml(item.status)}</p></div><div class="admin-row-actions"><button class="icon-button" data-move-department="${item.id}" data-direction="-1" ${index === 0 ? 'disabled' : ''}>↑</button><button class="icon-button" data-move-department="${item.id}" data-direction="1" ${index === data.departments.length - 1 ? 'disabled' : ''}>↓</button><a class="text-link" href="department.html?department=${encodeURIComponent(item.slug)}">View</a><button class="text-link" data-duplicate-department="${item.id}">Duplicate</button><button class="text-link" data-edit-department="${item.id}">Edit</button><button class="icon-button danger" data-delete-department="${item.id}">Delete</button></div></article>`).join('')}</div>`;
    ...
    root.querySelectorAll('[data-duplicate-department]').forEach(button => button.onclick = async () => { const source = data.departments.find(item => item.id === button.dataset.duplicateDepartment); const copy = structuredClone(source); copy.id = uid(); copy.slug = `${source.slug}-copy`; copy.name = `${source.name} copy`; await saveItem('departments', copy, data, 'Duplicated department'); renderAdminTab('departments'); });
