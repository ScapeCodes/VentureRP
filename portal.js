(() => {
  ...
  function initFiveMHome() {
    ...
    const launch = async () => {
      ...
      const launcher = document.createElement('a'); launcher.href = join.joinUrl; launcher.target = '_blank'; launcher.rel = 'noopener'; launcher.hidden = true; document.body.appendChild(launcher); launcher.click(); launcher.remove();
    };
    const update = async () => {
      ...
      state.className = `server-state ${online ? full ? 'server-state--queue' : 'server-state--online' : 'server-state--offline'}`; state.innerHTML = `<i></i>${online ? full ? 'Server full · Website queue active' : 'Server online' : 'Server unavailable'}`;
    ...
    const tickTimer = () => { if (!queue?.readyUntil) return; const seconds = Math.max(0, Math.ceil((queue.readyUntil - Date.now()) / 1000)); timer.textContent = `${Math.floor(seconds / 60)}:${String(seconds % 60).padStart(2, '0')}`; };
    button.onclick = async () => {
      if (!getSession()) { beginLogin('./'); return; }
      button.disabled = true;
      try { if (queue?.state === 'ready') await launch(); else queue = await request('/api/queue', { method: 'POST', body: '{}' }); await update(); } catch (error) { toast(error.message); button.disabled = false; }
    };
  ...
  async function initProfile() {
    ...
    const [response, playerStatus] = await Promise.all([
      request('/api/forms').catch(error => { toast(error.message); return null; }),
      request('/api/player-status').catch(() => null),
    ]);
    const store = demoData();
    ...
    const showTab = tab => {
      ...
      renderProfileContent(tab, { forms, submissions, store, session, preferences, playerStatus });
  ...
  function renderProfileContent(tab, context) {
    ...
    if (tab === 'overview') {
      ...
      const status = context.playerStatus;
      const inDiscord = status?.inDiscord ?? Boolean(session.guild);
      const allowlisted = status?.allowlisted ?? false;
      const allowlistLabel = allowlisted ? 'Allowlisted' : status?.allowlistConfigured ? 'Pending' : 'Not set';
      const memberSince = status?.memberSince ? new Date(status.memberSince).toLocaleDateString(getPreferences().language || 'en-GB', { day: 'numeric', month: 'short', year: 'numeric' }) : 'Connected today';
      root.innerHTML = `<div class="dashboard-welcome"><div><p class="eyebrow"><span></span> Welcome back</p><h2>OVERVIEW</h2><p>Everything you need for Venture, in one place.</p></div><div class="dashboard-user"><img src="${session.user.avatar ? `https://cdn.discordapp.com/avatars/${session.user.id}/${session.user.avatar}.png?size=96` : 'logo.png'}" alt="" /><span><b>${escapeHtml(profileName(session))}</b><small>@${escapeHtml(session.user.username)}</small></span></div></div><div class="player-bento"><article class="bento-card bento-card--discord${inDiscord ? ' bento-card--connected' : ''}"><span class="bento-icon">${inDiscord ? '✓' : '◉'}</span><small>Discord server</small><strong>${inDiscord ? 'Connected' : 'Not connected'}</strong><p>${inDiscord ? escapeHtml(session.guild?.name || 'Venture Roleplay') : 'Join Discord to get started.'}</p></article><article class="bento-card bento-card--allowlist"><span class="bento-icon">✓</span><small>Allowlist status</small><strong>${allowlistLabel}</strong><p>${allowlisted ? 'You can join the Venture queue.' : 'Your roles are checked when you enter the queue.'}</p></article><article class="bento-card"><span class="bento-icon">◷</span><small>Venture member since</small><strong>${memberSince}</strong><p>Discord account connected to Venture.</p></article><article class="bento-card"><span class="bento-icon">▣</span><small>Open applications</small><strong>${privateForms.length}</strong><p>${submissions.length} submission${submissions.length === 1 ? '' : 's'} tracked.</p></article><article class="bento-card bento-card--action"><div><small>Ready to play?</small><strong>ENTER THE QUEUE</strong><p>Check your status and reserve your place.</p></div><button data-go-tab="queue">Open queue →</button></article><article class="bento-card bento-card--activity"><div class="profile-subhead"><div><small>Recent activity</small><strong>Your submissions</strong></div><button class="text-link" data-go-tab="submissions">View all</button></div>${submissionRows(recent)}</article></div>`;
      ...
      root.innerHTML = `<div class="profile-section-heading member-forms-heading"><div><p class="eyebrow"><span></span> Confidential submissions</p><h2>MEMBER FORMS</h2><p>Appeals, reports and member requests are sent privately to the appropriate staff team.</p></div><div class="member-forms-count"><strong>${String(privateForms.length).padStart(2, '0')}</strong><span>Forms available</span></div></div><div class="member-form-grid">${privateForms.map((form, index) => { const hasDraft = localStorage.getItem(`venture_draft_${session.user.id}_${form.id}`); const questionCount = form.fields?.length || 0; return `<article class="member-form-card${hasDraft ? ' member-form-card--draft' : ''}"><header><span class="member-form-privacy"><i></i>${form.ticketEnabled ? 'Private ticket' : 'Private'}</span><span class="member-form-number">${String(index + 1).padStart(2, '0')}</span></header><div class="member-form-card__body"><small>${questionCount} ${questionCount === 1 ? 'question' : 'questions'}${hasDraft ? ' · Draft saved' : ''}</small><h3>${escapeHtml(form.title)}</h3><p>${escapeHtml(form.description)}</p></div><footer><span>${hasDraft ? 'Your progress is saved on this device' : form.ticketEnabled ? 'Continue the conversation from your profile' : 'Visible only to authorised staff'}</span><button class="button" data-member-form="${escapeHtml(form.id)}">${hasDraft ? 'Continue draft' : 'Start form'} <b>→</b></button></footer></article>`; }).join('') || '<div class="empty-state"><h3>No member forms are open</h3><p>Check back later.</p></div>'}</div>`;
  ...
  function submissionRows(submissions, interactive = false) {
    ...
    return submissions.map(item => `<${interactive ? 'button' : 'article'} class="profile-submission-row" ${interactive ? `data-submission="${escapeHtml(item.id)}"` : ''}><div><small>${new Date(item.updatedAt || item.createdAt).toLocaleDateString(locale)}${item.ticket ? ` · Ticket ${escapeHtml(item.ticketNumber || item.id.slice(0, 8))}` : ''}</small><strong>${escapeHtml(item.formTitle || item.formId)}</strong></div><span class="status-pill status-pill--${escapeHtml(item.status)}">${escapeHtml(statusLabel(item.status))}</span></${interactive ? 'button' : 'article'}>`).join('');
  }

  function openMemberSubmission(item) {
    if (item?.ticket) { openMemberTicket(item); return; }
  ...
  function ticketAnswersMarkup(ticket) {
    return `<div class="ticket-message ticket-message--member"><header><div><strong>${escapeHtml(ticket.user?.global_name || ticket.user?.username || 'Member')}</strong><span>Initial submission</span></div><time>${new Date(ticket.createdAt).toLocaleString()}</time></header><div class="answer-list">${Object.entries(ticket.values || {}).map(([key, value]) => `<div><small>${escapeHtml(key.replaceAll('_', ' '))}</small><p>${escapeHtml(value).replaceAll('\n', '<br />')}</p></div>`).join('')}</div></div>`;
  }

  function ticketMessagesMarkup(ticket) {
    return (ticket.messages || []).map(message => `<article class="ticket-message ticket-message--${message.senderType === 'staff' ? 'staff' : 'member'}"><header><div><strong>${escapeHtml(message.sender?.global_name || message.sender?.username || (message.senderType === 'staff' ? 'Staff' : 'Member'))}</strong><span>${message.senderType === 'staff' ? 'Staff reply' : 'Member reply'}</span></div><time>${new Date(message.createdAt).toLocaleString()}</time></header><p>${escapeHtml(message.body).replaceAll('\n', '<br />')}</p></article>`).join('');
  }

  function ticketHeaderMarkup(ticket) {
    return `<div class="ticket-heading"><div><p class="eyebrow"><span></span> Ticket ${escapeHtml(ticket.ticketNumber || ticket.id.slice(0, 8))}</p><h2>${escapeHtml(ticket.formTitle || ticket.formId)}</h2></div><span class="status-pill status-pill--${escapeHtml(ticket.status)}">${escapeHtml(statusLabel(ticket.status))}</span></div><div class="ticket-assignment"><small>Handling this ticket</small><strong>${ticket.claimedBy ? escapeHtml(ticket.claimedBy.name || ticket.claimedBy.global_name || ticket.claimedBy.username) : 'Waiting for a staff member to claim'}</strong></div>`;
  }
  ...
  async function renderAdminTab(tab) {
    ...
    else if (tab === 'tickets') renderTicketsAdmin(root, data);
  ...
  function renderSubmissionsAdmin(root, data) {
    ...
    const renderList = () => { const term = search.value.trim().toLowerCase(); const visible = data.submissions.filter(item => !item.ticket && (hasScoped('submissions.view', item.formId) || hasScoped('submissions.manage', item.formId)) && (filter.value === 'all' || item.status === filter.value) && (!term || JSON.stringify(item).toLowerCase().includes(term))); list.innerHTML = visible.map(item => `<article><div><span class="status-pill status-pill--${escapeHtml(item.status)}">${escapeHtml(statusLabel(item.status))}</span><h3>${escapeHtml(item.formTitle || item.formId)}</h3><p>${escapeHtml(item.user?.global_name || item.user?.username || 'Unknown member')} · ${new Date(item.createdAt).toLocaleString()}${item.assignedTo ? ` · Assigned to ${escapeHtml(item.assignedTo)}` : ''}</p></div><button class="text-link" data-view-submission="${item.id}">${hasScoped('submissions.manage', item.formId) ? 'Review' : 'View'}</button></article>`).join('') || '<div class="empty-state"><h3>No matching submissions</h3></div>'; list.querySelectorAll('[data-view-submission]').forEach(button => button.onclick = () => openSubmissionReview(data.submissions.find(item => item.id === button.dataset.viewSubmission), data)); };
    search.oninput = renderList; filter.onchange = renderList; renderList(); initIcons();
  }

  function renderTicketsAdmin(root, data) {
    root.innerHTML = adminHeading('TICKETS') + `<div class="admin-note ticket-queue-note"><strong>Form-scoped queue</strong><p>You only see tickets for forms your Discord roles can view or manage. A staff member must claim a ticket before replying or changing its status.</p></div><div class="admin-filterbar"><label class="suggestion-search"><span class="icon" data-icon="search"></span><input id="ticket-search" placeholder="Search tickets, forms or members…" /></label><select id="ticket-filter"><option value="all">All statuses</option><option value="open">Open</option><option value="claimed">Claimed</option><option value="pending">Waiting</option><option value="resolved">Resolved</option><option value="closed">Closed</option></select></div><div class="admin-list ticket-admin-list" id="ticket-admin-list"></div>`;
    const list = root.querySelector('#ticket-admin-list'); const search = root.querySelector('#ticket-search'); const filter = root.querySelector('#ticket-filter');
    const renderList = () => {
      const term = search.value.trim().toLowerCase();
      const visible = data.submissions.filter(item => item.ticket && (hasScoped('submissions.view', item.formId) || hasScoped('submissions.manage', item.formId)) && (filter.value === 'all' || item.status === filter.value) && (!term || JSON.stringify(item).toLowerCase().includes(term))).sort((a, b) => new Date(b.updatedAt || b.createdAt) - new Date(a.updatedAt || a.createdAt));
      list.innerHTML = visible.map(item => `<article><div><div class="ticket-list-meta"><span class="status-pill status-pill--${escapeHtml(item.status)}">${escapeHtml(statusLabel(item.status))}</span><span>${escapeHtml(item.ticketNumber || item.id.slice(0, 8))}</span></div><h3>${escapeHtml(item.formTitle || item.formId)}</h3><p>${escapeHtml(item.user?.global_name || item.user?.username || 'Unknown member')} · Updated ${new Date(item.updatedAt || item.createdAt).toLocaleString()} · ${item.claimedBy ? `Claimed by ${escapeHtml(item.claimedBy.name || item.claimedBy.global_name || item.claimedBy.username)}` : 'Unclaimed'}</p></div><button class="text-link" data-view-ticket="${escapeHtml(item.id)}">${hasScoped('submissions.manage', item.formId) ? 'Manage' : 'View'}</button></article>`).join('') || '<div class="empty-state"><h3>No matching tickets</h3><p>New ticket-enabled form submissions will appear here.</p></div>';
      list.querySelectorAll('[data-view-ticket]').forEach(button => button.onclick = () => openStaffTicket(visible.find(item => item.id === button.dataset.viewTicket)));
    };
    search.oninput = renderList; filter.onchange = renderList; renderList(); initIcons();
  }
  ...
  function openRoleEditor(existing, data) {
    ...
    const formScopes = data.forms.map(form => `<section class="resource-permission-group"><h3>${escapeHtml(form.title)}</h3>${permissionOption(`forms.manage:${form.id}`, 'Edit this form', 'Edit or delete this form only')}${permissionOption(`submissions.view:${form.id}`, 'View submissions and tickets', 'Read submissions and ticket conversations for this form')}${permissionOption(`submissions.manage:${form.id}`, 'Manage submissions and tickets', 'Claim tickets, reply, update status, and review submissions')}</section>`).join('');
